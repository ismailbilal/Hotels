...
export const createHasVisited = async (userId, hotelId) => {
  const res = await fetch(`${API_BASE}/user/${userId}/hotel/${hotelId}`, {
    method: "POST",
    mode: "cors",
    body: JSON.stringify({}),
    headers: {
...
export const isVisitedBy = async (userId, hotelId) => {
  const res = await fetch(
    `${API_BASE}/user/${userId}/hasvisit/hotel/${hotelId}`
  );
  const data = await res.json();
  return data;
};

export const getHotelsVisited = async (userId) => {
  const res = await fetch(`${API_BASE}/user/${userId}/hotel`);
  const data = await res.json();
  return data;
};
